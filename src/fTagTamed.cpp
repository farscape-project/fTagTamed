...
int main(int argc, char **argv)
{
  ...
  if( !(argc == 6 || argc == 7) ) {
    std::cerr << "usage: fTagTamed cone_angle smoothing_lambda tag_offset input_stl_file output_tag_file [output_msh_file]" << std::endl;
    std::cerr << ""                                                                                                         << std::endl;
    std::cerr << "  cone_angle:       the aperture of the ray sampling cone (floating-point in [0, 180[)"                   << std::endl;
  ...
  Polyhedron mesh;
  if( !CGAL::IO::read_STL(argv[4], mesh) || !CGAL::is_triangle_mesh(mesh) ) {
  ...
  tag_file.open(argv[5], std::ios_base::out | std::ios_base::app);
  for( facet f : mesh.facet_handles() )
    tag_file << std::stoi(argv[3]) + facet_segment_map[f] << std::endl;
  ...
  msh_file.open(argv[6], std::ios_base::out | std::ios_base::trunc);
  ...
  for( facet f : mesh.facet_handles() ) {
    msh_file << f->id() << " 2 2"
              << " " << std::stoi(argv[3]) + facet_segment_map[f]
              << " " << std::stoi(argv[3]) + facet_segment_map[f];
